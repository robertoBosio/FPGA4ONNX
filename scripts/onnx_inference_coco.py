...
def coco_dataloader(
    ...
    dataset = COCODataset(image_root, annotations, transform)
...
class ZCU102PowerMonitor:
    def __init__(self, rail_names: set[str] | None = None):
    ...
    def _discover_rails(self) -> list[INARail]:
        ...
        for entry in os.listdir(directory):
            ...
            if matched_rail is None:
                continue
            if self.rail_names is not None and matched_rail not in self.rail_names:
                continue
            ...
            if os.path.isfile(current_path) and os.path.isfile(voltage_path):
                rails.append(
                    INARail(
                        ...
                        power_path=power_path if os.path.isfile(power_path) else None,
                    )
                )
    ...
    def read_total_power_w(self) -> float:
        ...
        for rail in self.rails:
            total_w += self._read_rail_power_w(rail)
        return total_w

    def read_rail_powers_w(self) -> dict[str, float]:
        out = {}
        for rail in self.rails:
            out[rail.rail] = self._read_rail_power_w(rail)
        return out

    def _read_rail_power_w(self, rail: INARail) -> float:
        if rail.power_path is not None:
            # Linux hwmon exposes INA226 power1_input in microwatts.
            return self._read_float(rail.power_path) / 1_000_000.0
        curr_ma = self._read_float(rail.current_path)
        volt_mv = self._read_float(rail.voltage_path)
        return (curr_ma * volt_mv) / 1_000_000.0
...
def benchmark_preloaded_concurrent(
    ...
    for batch_idx, np_features in enumerate(batches):
        if int(np_features.shape[0]) != 1:
            raise ValueError(
                f"Concurrent accelerator benchmark requires batch size 1, "
...
def write_optimized_results_file(filepath: str, opt: dict):
    with open(filepath, "w", encoding="utf-8") as f:
        f.write("================ Optimized benchmark ================\n")
        f.write(f"Throughput:          {opt['throughput_img_s']:.2f} img/s\n")
        f.write(f"Average latency:     {opt['avg_img_ms']:.3f} ms\n")
        f.write(f"P50 latency:         {opt['p50_img_ms']:.3f} ms\n")
        f.write(f"P90 latency:         {opt['p90_img_ms']:.3f} ms\n")
        f.write(f"P95 latency:         {opt['p95_img_ms']:.3f} ms\n")
        f.write("=====================================================\n\n")
        f.write("Raw metrics (JSON):\n")
        json.dump({"optimized": opt}, f, indent=2)
        f.write("\n")
...
def run_speed(args) -> int:
    ...
    input_name = sess_opt.get_inputs()[0].name
    ...
    if orig_results is not None:
        ...
        write_optimized_results_file(args.results_file, opt_results)
...
def run_speed_with_power(args, custom_op_so: str, batches: list[np.ndarray]) -> int:
    ...
    input_name = sess_opt_warmup.get_inputs()[0].name
    if sess_orig_warmup is not None:
    ...
    power_rails = None
    if args.power_rails:
        power_rails = {rail.strip() for rail in args.power_rails.split(",") if rail.strip()}
        unknown_rails = sorted(power_rails - set(RAILNAME_MAP))
        if unknown_rails:
            raise ValueError(f"Unknown power rails: {unknown_rails}. Known rails: {sorted(RAILNAME_MAP)}")
        print(f"Recording selected power rails: {', '.join(sorted(power_rails))}")
    else:
        print("Recording all discovered power rails")

    power_monitor = ZCU102PowerMonitor(rail_names=power_rails)
    ...
    sync_perf_ns_orig: int | None = None
    sync_perf_ns_opt: int | None = None
    orig_results = None
    opt_results = None
    ...
    if profile_file_orig and profile_file_opt and sync_perf_ns_orig is not None and sync_perf_ns_opt is not None:
        build_merged_trace(
...
def parse_args():
    ...
    parser.add_argument("--model", default="accelerated_yolov5nu.onnx")
    parser.add_argument("--original-model", default="original_model_qcdq.onnx")
    parser.add_argument("--custom-op", default="libcustomop.so")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="ONNX Runtime intra-op thread count. Use 0 for ORT default.",
    )
    parser.add_argument(
        "--inter-op-threads",
        type=int,
        default=0,
        help="ONNX Runtime inter-op thread count. Use 0 for ORT default; values >1 enable ORT_PARALLEL.",
    )
    parser.add_argument("--results-file", default="performance_improvement.txt")
    parser.add_argument(
        "--profile",
        action="store_true",
        help="Enable ONNX Runtime profiling in speed mode.",
    )
    parser.add_argument(
        "--skip-original",
        action="store_true",
        help="In speed mode, benchmark only the optimized model.",
    )
    parser.add_argument(
        "--power-record",
        action="store_true",
    ...
    parser.add_argument(
        "--power-rails",
        default="",
        help="Comma-separated ZCU102 rails to record. Default records all discovered rails.",
    )
    ...
    if args.intra_op_threads < 0:
        parser.error("--intra-op-threads must be >= 0")
    if args.inter_op_threads < 0:
        parser.error("--inter-op-threads must be >= 0")
